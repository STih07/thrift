...
var HttpConnection = exports.HttpConnection = function (host, port, options) {
    ...
    this.nodeOptions = {
        ...
        url: (this.https ? 'https://' : 'http://') + this.host + ':' + this.port + this.options.path || '/',
        method: 'POST',
    ...
    this.responseCallback = function (response) {
        var data = [];
        var dataLen = response.content.length;
        ...
        self.transport.receiver(decodeCallback)(Buffer.from(response.content.toString(), 'base64'));
    };
};
